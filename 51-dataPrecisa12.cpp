...
class DataPrecisa: Data, Cronometro{
    public:
        DataPrecisa(int, int, int, int, int, int, char);
        void printar();
        void incrementar();
};

DataPrecisa::DataPrecisa(int dia, int mes, int ano, int segundos, int minutos, int horas, char PM): Data(dia, mes, ano), Cronometro(segundos, minutos, horas, PM){}

void DataPrecisa::printar(){
    cout << this->getDia() << "/" << this->getMes() << "/" << this->getAno() << " " << this->getHoras() << ":" << this->getMinutos() << ":" << this->getSegundos() << " " << this->getPM() << "M" << endl;
}

void DataPrecisa::incrementar(){
    int horaAnterior = this->getHoras();
    this->Cronometro::incrementar();
    if(horaAnterior == 11 and this->getHoras() == 12 and this->getPM() == 'A'){
...
int main() {
    ...
    char operacao, PM;
    cin >> dia >> mes >> ano >> horas >> minutos >> segundos >> PM;
    DataPrecisa *objDataPrecisa = new DataPrecisa(dia, mes, ano, segundos, minutos, horas, PM);
    cin >> operacao;
    while(cin >> operacao){
