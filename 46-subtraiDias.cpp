...
void Data::diaAnterior(){
    Data *novaData = new Data(this->getDia()-1, this->getMes(), this->getAno());
    if(!novaData->isValida()){
        novaData->setDia(31);
        novaData->setMes(novaData->getMes()-1);
        while (!novaData->isValida() and novaData->getDia() > 28){
            novaData->setDia(novaData->getDia()-1);
        }
        if(!novaData->isValida()){
            novaData->setDia(31);
            novaData->setMes(12);
            novaData->setAno(novaData->getAno()-1);
        }
    }
...
int main() {
    int dia, mes , ano, qtdDiasAMenos;
    Data *objData;
    while(cin >> dia >> mes >> ano >> qtdDiasAMenos){
    	objData = new Data(dia, mes, ano);
        objData = *objData - qtdDiasAMenos;
        objData->printarData();
    }
    return 0;
}